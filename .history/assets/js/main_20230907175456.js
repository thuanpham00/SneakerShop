...
window.addEventListener("load", function () {
    ...
    // seen-ins scroll
    const seenIns_list = document.querySelector(".seen-ins__list")
    ...
    buttonSeen__left.addEventListener("click", function(e) {
        const b = seenIns_list.scrollLeft;
        seenIns_list.scroll(b - 800, 0);
        // if(seenIns_list.scrollLeft > 0) {
        //     buttonSeen__left.classList.remove("hidden")
        // } 
    })
    buttonSeen__right.addEventListener("click", function(e) {
        const b = seenIns_list.scrollLeft;
        seenIns_list.scroll(b + 800, 0) // cộng vào tọa độ
        console.log(b);
        if(seenIns_list.scrollLeft > 0) {
            buttonSeen__left.classList.remove("hidden")
        } else if(seenIns_list.scrollLeft > 3000) {
            buttonSeen__right.classList.add("hidden")
            buttonSeen__right.classList.remove("hidden")
        }
    })
});
