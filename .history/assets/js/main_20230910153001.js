...
window.addEventListener("load", function () {
    ...
    [...rcmImg].forEach((item) =>
        item.parentNode.parentNode.addEventListener("click", function (e) {
            ...
            [...sizeUS].forEach(item => item.addEventListener("click", function(e) {
                const textSize = item.querySelector(".modal-tableSize__size.sizeUS").textContent;
                const textSize2 = textSize.textContent;
                const modalInfoAll = item.parentNode.parentNode.previousElementSibling;
                const All = modalInfoAll.querySelector(".modal-info__all span")
                // const sizeAll = size.querySelector(".modal-tableSize__size");
                // sizeAll.textContent = textSize;
                console.log(All.te)
                // All.textContent = textSize2
            }))
        })
    );
    // remove modal
