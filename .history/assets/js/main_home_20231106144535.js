...
<div class="modal__priceProduct">
    ...
    // date now: Sun Sep 24 2023 21:37:00 GMT+0700 (Indochina Time)
    function countDownTimer(date) {
        let start = new Date().getTime();
        const endDate = new Date(date).getTime();
        if (isNaN(endDate) || endDate - start <= 0) {
            return;
        }
        ...
        function countDown() {
            ...
            let timer = parseInt((endDate - startDate) / 1000);
            if (timer > 0) {
                ...
                // ví dụ có 1 chữ số 5 thì -> 05 ví dụ có 2 chữ số 45 thì 045 lấy -2 thì 45
    ...
    countDownTimer("Sat Dec 9 2023 21:37:00 GMT+0700 (Indochina Time)");
});
...
document.addEventListener("DOMContentLoaded", function () {
    const loggedInUser = localStorage.getItem("loggedInUser"); // kiểm tra localStorage
    if (loggedInUser) { // nếu có
        const accountInfoElement = document.getElementById("accountInfo");
        accountInfoElement.style.display = "block";
        const accountNameElement = document.getElementById("accountName");
        accountNameElement.textContent = loggedInUser;
        const loginButtonElement = document.querySelector(".button-login");
        ...
        logoutLinkElement.addEventListener("click", function (e) {
            e.preventDefault();
            sessionStorage.removeItem("loggedInUser"); // xóa lcStr đi
