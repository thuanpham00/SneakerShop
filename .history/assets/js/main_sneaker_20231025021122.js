...
async function getProductSneaker(link = endPointSneaker, page = 1) {
    const response = await fetch(`${link}&_page=${page}`);
...
const numberPage = document.querySelector(".link-number");
numberPage.addEventListener("click", async function(e) {
    page++;
    await getProductSneaker(link,page)
