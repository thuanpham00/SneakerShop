...
window.addEventListener("load", function () {
    ...
    function showSuccess(input) {
        ...
        [...small].forEach(item => item.textContent = "");
    }

    function showError(input, message) {
        const small = document.querySelector("small");
        [...small].forEach(item => item.textContent = message);
    ...
    function checkLength(input, min, max) {
        if (input.value.length < min) {
            showError(
                input,
                `${getFieldName(input)} tối thiểu có ${min} ki tu`
            );
        } else if (input.value.length > max) {
            showError(input, `${getFieldName(input)} tối đa có ${max} ki tu`);
        } else {
            showSuccess(input);
        }
    }
    ...
    function checkEmail(input) {
        ...
        if (regex.test(input.value)) {
            ...
            showError(input, `${getFieldName(input)} not a valid`);
        }
    }
