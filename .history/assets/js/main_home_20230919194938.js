...
// Hàm để tạo một đối tượng mục trong giỏ hàng và thêm vào mảng
function createItemCartObject(image, nameCart, amountCart, priceAddCart) {
  const item = {
    image: image,
    name: nameCart,
    amount: amountCart,
...
// Hàm để lưu trữ mảng các mục trong giỏ hàng vào localStorage
function saveCartItems() {
  localStorage.setItem("cartItems", JSON.stringify(cartItems));
}

// Hàm để tải dữ liệu từ localStorage khi trang web được tải lại
...
// Gọi hàm loadCartItems khi trang web được tải lại
window.addEventListener("DOMContentLoaded", function (e) {
  loadCartItems();
});

// Gọi hàm createItemCartObject khi click vào một mục trong giỏ hàng
const getImg2 = getImg.getAttribute("src");
const nameCart = document.querySelector(".modal__heading").textContent;
const amountCart = document.querySelector(".priceAmount").textContent;
const priceAddCart = document.querySelector(".moneyCart.Cart-1").textContent;

createItemCartObject(getImg2, nameCart, amountCart, priceAddCart);

// Duyệt qua mảng các mục trong giỏ hàng và hiển thị thông tin
