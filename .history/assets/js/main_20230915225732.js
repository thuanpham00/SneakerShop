...
<div class="modal__priceProduct">
    ...
    [...rcmImg].forEach((item) =>
        item.parentNode.parentNode.addEventListener("click", function (e) {
            ...
            // // Lưu đoạn mã vào localStorage
            // function saveHTMLToLocalStorage(html) {
            //     const savedHTMLs =
            //         JSON.parse(localStorage.getItem("savedHTMLs")) || [];
            //     savedHTMLs.push(html);
            //     localStorage.setItem("savedHTMLs", JSON.stringify(savedHTMLs));
            // }

            // // Khôi phục đoạn mã từ localStorage
            // function restoreHTMLFromLocalStorage() {
            //     const savedHTMLs =
            //         JSON.parse(localStorage.getItem("savedHTMLs")) || [];
            //     savedHTMLs.forEach((html) => {
            ...
            //     function createItemCart(image, nameCart, amountCart) {
            //         const templateCart = `<div class="addCart__item">
            //     <img src="${image}" alt="" class="addCart__img">
            //     <div class="addCart__info">
            //         <h4 class="addCart__name line-clamp">
            //             ${nameCart}
            //         </h4>
            //         <div class="addCart__amount">
            //             <span class="addCart__Number">${amountCart}</span>
            //             <span class="addCart__x">x</span>
            //         </div>
            //     </div>
            // </div>`;
            //         cartList.insertAdjacentHTML("afterbegin", templateCart);
            //         saveHTMLToLocalStorage(templateCart);
            //     }

            //     createItemCart(getImg2, nameCart, amountCart2);
            // });
            // window.addEventListener("DOMContentLoaded", restoreHTMLFromLocalStorage);

            // new
            // Lưu đoạn mã vào localStorage
            // Đảm bảo các biến và phần tử DOM được khai báo trước khi sử dụng
            const cartList = document.querySelector(".addCart__list");
            ...
            addToCard.addEventListener("click", function (e) {
                ...
                const existingCartItem = cartList.querySelector(
                    `.addCart__name:contains('${nameCart}')`
                );
                if (!existingCartItem) {
                    createItemCart(getImg2, nameCart, amountCart2);
                }
        ...
        <div class="addCart__info">
            ...
            <div class="addCart__amount">
                ...
                cartList.insertAdjacentHTML("afterbegin", templateCart);
                saveHTMLToLocalStorage(templateCart); // Lưu HTML vào Local Storage sau khi chèn vào cartList
            }

            // Đảm bảo rằng restoreHTMLFromLocalStorage được gọi sau khi DOM đã được tải
            window.addEventListener(
                "DOMContentLoaded",
                restoreHTMLFromLocalStorage
            );
        })
    );
