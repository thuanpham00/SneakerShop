    ...
    [...rcmImg].forEach((item) =>
        item.parentNode.parentNode.addEventListener("click", function (e) {
            ...
            addToCard.addEventListener("click", function (e) {
                ...
                localStorage.setItem("addCartItem", JSON.stringify(addCartItemHTML))
                const storeAddCartItemHtml = localStorage.getItem("addCartItem");
                const storeAddCartItem = JSON.parse(storeAddCartItemHtml);

                if(storeAddCartItem) {
                    const cartList = document.querySelector(".add__list")
                }
            });
        })
    );
