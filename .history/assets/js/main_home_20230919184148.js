    ...
    [...rcmImg].forEach((item) =>
        item.parentNode.parentNode.addEventListener("click", function (e) {
            ...
            addToCard.addEventListener("click", function (e) {
                ...
                const cartArray = [];
                function createItemCart2(image, nameCart, amountCart, priceAddCart) {
                    const item = {
                        image: image,
                        name: nameCart,
                        amount: amountCart,
                ...
                window.addEventListener("DOMContentLoaded",function(e) {
                    const storedCartArray = this.localStorage.getItem("cartArray");
                    const parsedCartArray = storedCartArray.parse(storedCartArray);

                    if(parsedCartArray) {
                        cartArray.push(...parsedCartArray)
                    }
                })
                createItemCart2(getImg2, nameCart, amountCart2, priceAddCart)
            });
        })
    );
