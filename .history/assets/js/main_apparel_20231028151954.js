...
let currentPage = 1;
const limit = 10;
const endPointShoes = `https://dm92v6-8080.csb.app/productApparel?_limit=${limit}`;
const apparelList = document.querySelector(".apparel-list");
function renderItemApparel(item) {
    ...
    apparelList.insertAdjacentHTML("beforeend", template);
}
// lấy dữ liệu từ json và get ra giao diện
async function getProductApparel(page = 1) {
    const response = await fetch(`${endPointShoes}&_page=${currentPage}`);
    const newData = await response.json();
    loading.style.display = "block";
    data = newData; // lưu cái data vào 1 mảng
    apparelList.innerHTML = ""; // trước khi render product thì nội dung của nó rỗng
    if (data.length > 0 && Array.isArray(data)) {
        loading.style.display = "none";
        data.forEach((item) => {
            renderItemApparel(item);
        });
    }
}
getProductApparel();
...
const apparelList1 = document.querySelector(".apparel-list");
apparelList1.addEventListener("click", function (e) {
    if (e.target.matches(".sneaker__item")) {
        // click vào item nào thì từ item đó trỏ vào phần tử bên trong
        // xử lý hình
        const imageModal = e.target.querySelector(".sneaker__pic");
        const imageModal2 = imageModal.getAttribute("src");

        // xử lý name
        const nameProduct = e.target.querySelector(".sneaker__title");
        const nameProductModal = nameProduct.textContent;

        // // xử lý giá
        const priceRcm = e.target.querySelector(".sneaker__buy").textContent;
...
async function getProductFilter(link = endPointShoes) {
    const response = await fetch(link);
    const data = await response.json();
    apparelList.innerHTML = "";
    if(data.length > 0 && Array.isArray(data)) {
        data.forEach(item => {
            renderItemApparel(item)
        })
...
filterInput.addEventListener("keydown", function (e) {
    let path = endPointShoes;
    if(e.target.value !== "") {
        path = `${endPointShoes}&title_likêe=${e.target.value}`
    }
